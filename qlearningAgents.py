...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        "*** YOUR CODE HERE ***"
        actions = self.getLegalActions(state)
        stateQValue = self.computeValueFromQValues(state)

        maxAction = []
        if len(actions) == 0:
            return None
        else:
            maxAction = [action for action in actions if self.getQValue(state, action) == stateQValue]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for i in features.keys():
            qValue = qValue + self.weights[i] * features[i]
        return qValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        qValueNextState = self.computeValueFromQValues(nextState)
        difference = (reward + self.discount*(qValueNextState))- qValueState
